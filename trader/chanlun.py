...
class Kline:
    def __init__(self, index: int, date: datetime, h: float, l: float, o: float, c: float, a: float,
                 amount: float, preClose: float):
        self.index: int = index
        self.date: datetime = date
        self.h: float = round(h, 2)
        self.l: float = round(l, 2)
        self.o: float = round(o, 2)
        self.c: float = round(c, 2)
        self.a: float = round(a, 2)
        self.amount: float = amount      # 新增成交额
        self.preClose: float = round(preClose, 2)  # 新增前收盘价
        self.zt_price : float = round(preClose * 1.1 , 2)   # 涨停价（按A股规则）
        self.is_long_gun = False     # 长枪标记
        self.is_yizi = False         # 一字板标记
        self.is_t_zi = False         # T字板标记
        self.is_zt = False           # 普通涨停标记
        self.zt_type = None          # 涨停类型
        self.patterns = []           # 组合形态
        # print(f'index{self.index},date{self.date},h{self.h},l{self.l},o{self.o},c{self.c},a{self.a},amount{self.amount},prevolume{self.preClose},zt_price {self.zt_price}')

    def __str__(self):
        return "index: %s date: %s h: %s l: %s o: %s c:%s a:%s amount:%s preClose:%s" % \
               (self.index, self.date, self.h, self.l, self.o, self.c, self.a, self.amount, self.preClose)

    def check_patterns(self, prev_k1=None, prev_k2=None):
        """判断单根K线形态"""
        # 长枪判断
        if self.o > self.c:
            amplitude = (self.h - self.o) / self.o
        else:
            amplitude = (self.h - self.c) / self.c
        self.is_long_gun = amplitude >= 0.03

        # 一字板
        if all([self.o >= self.zt_price, self.c >= self.zt_price,
                self.h >= self.zt_price, self.l >= self.zt_price]):
            self.is_yizi = True
            self.zt_type = 'yizi'

        # T字板
        elif all([self.o >= self.zt_price, self.c >= self.zt_price,
                  self.h >= self.zt_price, self.l < self.zt_price]):
            self.is_t_zi = True
            self.zt_type = 't_zi'

        # 普通涨停
        elif self.c >= self.zt_price:
            self.is_zt = True
            self.zt_type = 'common'

        # 组合形态判断（需传入前两根K线）
        if prev_k1 and prev_k2:
            # 板后长枪
            if self.is_long_gun and prev_k1.zt_type:
                self.patterns.append('板后长枪')

            # 炸板长枪
            if self.is_long_gun and self.h >= self.zt_price:
                self.patterns.append('炸板长枪')

            # 趋势长枪
            if (self.is_long_gun and
                    prev_k2.c < prev_k1.c < self.c):
                self.patterns.append('趋势长枪')

class CLKline:
    def __init__(self, k_index: int, date: datetime, h: float, l: float, o: float, c: float, a: float,
                 amount: float, preClose: float, klines: List[Kline] = [], index: int = 0, _n: int = 0, _q: bool = False):
        ...
        self.c: float = c
        self.a: float = a
        self.amount: float = amount       # 新增
        self.preClose: float = preClose   # 新增
        self.klines: List[Kline] = klines
        self.index: int = index
        self._n: int = _n
        self._q: bool = _q
...
class CL:
    ...
    def process_cl_datas(self):
        ...
        self.process_mmds()
        self.process_kline_patterns()

    def process_kline_patterns(self):
        for i in range(len(self.klines)):
            k = self.klines[i]
            prev1 = self.klines[i-1] if i>0 else None
            prev2 = self.klines[i-2] if i>1 else None
            k.check_patterns(prev1, prev2)
    ...
    def process_cl_kline(self, klines: pd.DataFrame):
        ...
        for i in range(len(klines)):
            ...
            k = Kline(
                ...
                o=float(nk['open']), c=float(nk['close']),
                a=float(nk['volume']),
                amount=float(nk['amount']),  # 新增
                preClose=float(nk['preClose'])  # 新增
            )
    ...
    def __create_cl_kline(self, k: Kline) -> Tuple[bool, CLKline]:
        ...
        """
        cl_k = CLKline(date=k.date,k_index=k.index,h=k.h,l=k.l,o=k.o,c=k.c,a=k.a,amount=k.amount,preClose=k.preClose,klines=[k],index=0,_n=1,_q=False)
            ...
            if up_kline.l > k.h or up_kline.h < k.l:
                ...
                self.cl_klines.append(
                    CLKline(date=k.date, k_index=k.index,
                            ...
                            l=min(k.h, self.klines[k.index - 1].h), o=k.o, c=k.c, a=k.a, amount=k.amount, preClose=k.preClose, klines=[], _n=1, _q=True)
                )
        ...
        if (up_k.h >= cl_k.h and up_k.l <= cl_k.l) or (cl_k.h >= up_k.h and cl_k.l <= up_k.l):
            if qushi == 'up':  # 趋势上涨，向上合并
                ...
                up_k.a += cl_k.a  # 交易量累加
                up_k.amount += cl_k.amount  # 累加成交额
                up_k.preClose = cl_k.preClose  # 更新前收盘价为最新值
            else:
